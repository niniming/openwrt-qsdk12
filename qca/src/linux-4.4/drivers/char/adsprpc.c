	...
	struct fastrpc_mmap **ppmap)
{
	...
	if (mflags == ADSP_MMAP_HEAP_ADDR ||
		...
		if (map->attr & FASTRPC_ATTR_NON_COHERENT ||
			...
			dma_set_attr(DMA_ATTR_FORCE_NON_COHERENT, &attrs);
...
static int fastrpc_buf_alloc(struct fastrpc_file *fl, size_t size,
				struct dma_attrs dma_attr, uint32_t rflags,
...
static int get_args(uint32_t kernel, struct smq_invoke_ctx *ctx)
{
	...
	if (lrpralen) {
		err = fastrpc_buf_alloc(ctx->fl, lrpralen, ctx_attrs, 0, 0, &ctx->lbuf);
		if (err)
			goto bail;
	}
	if (ctx->lbuf->virt)
				   ...
				   struct fastrpc_ioctl_invoke_crc *inv)
{
	...
	VERIFY(err, !(err = fastrpc_invoke_send(ctx, kernel, invoke->handle)));
	PERF_END);
					...
					size_t size, uintptr_t *raddr)
{
	...
	*raddr = (uintptr_t)routargs.vaddrout;
	...
	if (flags == ADSP_MMAP_HEAP_ADDR) {
		err = qcom_scm_lock_subsys_mem(TZ_PIL_AUTH_QDSP6_PROC,
					uint64_to_ptr(phys), size);
...
/*
* This function gives HLOS the access persmissions from DSP for the
* remoteheap memory region.
*/
						...
						size_t size, uint32_t flags)
{
	...
	if (flags == ADSP_MMAP_HEAP_ADDR) {
		...
		if (err)
			goto bail;
		err = qcom_scm_unlock_subsys_mem(TZ_PIL_AUTH_QDSP6_PROC,
					uint64_to_ptr(phys), size, routargs.skey);
	} else if (flags == ADSP_MMAP_REMOTE_HEAP_ADDR) {
		VERIFY(err, !hyp_assign_phys(phys, (uint64_t)size,
					me->channel[fl->cid].rhvm.vmid,
					me->channel[fl->cid].rhvm.vmcount,
...
bail:
	if (err ) {
		panic("adsprpc: rpmsg probe of %s cid %d failed\n",
...
static int fastrpc_channel_open(struct fastrpc_file *fl)
{
	...
	if (cid == ADSP_DOMAIN_ID && me->channel[cid].ssrcount !=
		...
		me->channel[cid].prevssrcount = me->channel[cid].ssrcount;
	}
#endif
bail:
	mutex_unlock(&me->smd_mutex);
...
static int fastrpc_cb_probe(struct device *dev)
{
	...
	VERIFY(err, !IS_ERR_OR_NULL(sess->smmu.mapping =
				...
				&platform_bus_type, start, MAX_SIZE_LIMIT)));
	if (err)
		goto bail;
