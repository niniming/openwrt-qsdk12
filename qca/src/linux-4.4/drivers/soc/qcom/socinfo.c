/*
 * Copyright (c) 2009-2018, 2019 The Linux Foundation. All rights reserved.
 *
...
msm_get_vendor(struct device *dev,
		...
		char *buf)
{
	return snprintf(buf, PAGE_SIZE, "QTI\n");
}
...
static void * __init setup_dummy_socinfo(void)
{
	if (early_machine_is_apq8084()) {
		...
		strlcpy(dummy_socinfo.build_id, "ipq6028 - ",
			sizeof(dummy_socinfo.build_id));
	} else if (early_machine_is_ipq6000()) {
		dummy_socinfo.id = 421;
		strlcpy(dummy_socinfo.build_id, "ipq6000 - ",
			sizeof(dummy_socinfo.build_id));
